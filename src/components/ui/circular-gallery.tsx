...
img.src = this.image;
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
    ...
    // Check if this image is centered
    this.isCentered = Math.abs(x) < this.width * 0.3;

    // Apply smooth scale based on center position with easing
    ...
    const scaleFactor = 1.15 - (eased * 0.25);
...
class App {
  ...
  createMedias(
    ...
    this.medias = this.mediasImages.map((data, index) => {
      return new Media({
        ...
        borderRadius,
        font,
        onImageClick: (clickedIndex: number) => this.scrollToImage(clickedIndex)
      });
    });
  }

  scrollToImage(index: number) {
    if (!this.medias || !this.medias[index]) return;
    const targetMedia = this.medias[index];
    const targetScroll = targetMedia.x + targetMedia.extra;
  ...
  onTouchUp() {
    ...
    // Remove move listeners when mouse is up
    const canvas = this.gl.canvas as HTMLCanvasElement;
  ...
  onClick(e: MouseEvent) {
    ...
    // Raycasting to detect clicked image
    const rect = (this.gl.canvas as HTMLCanvasElement).getBoundingClientRect();
    const x = ((e.clientX - rect.left) / rect.width) * 2 - 1;
    const y = -((e.clientY - rect.top) / rect.height) * 2 + 1;

    // Find the media closest to the click and centered enough
    let closestMedia: Media | null = null;
    let closestDistance = Infinity;

    this.medias.forEach(media => {
      const screenX = media.plane.position.x;
      const distance = Math.abs(screenX);
      
      // Only consider images that are somewhat visible
      if (distance < this.viewport.width / 2 && distance < closestDistance) {
    ...
    if (closestMedia && closestMedia.onImageClick) {
      closestMedia.onImageClick(closestMedia.index);
    }
  }
  ...
  addEventListeners() {
    ...
    // Add global mouseup to handle when mouse is released outside canvas
