...
const TestResults = ({ 
  ...
  const generateAIReview = () => {
    ...
    return { score, incorrectQuestions, errorsBySection };
  };
  ...
  // ── AI Review Screen ──
  if (showAIReview) {
    const { errorsBySection, incorrectQuestions } = generateAIReview();
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground mb-2">
                              {section === "Fill in the Blanks" || section === "Grammar" ? (
                                <>Found {questions.length} mistake{questions.length > 1 ? "s" : ""} in grammar rules. Focus on <strong>verb tenses, subject-verb agreement, and proper word usage</strong>.</>
                              ) : section === "Analogy" ? (
                                <>Identified {questions.length} analogy error{questions.length > 1 ? "s" : ""}. Work on <strong>understanding relationships between concepts</strong> and pattern recognition.</>
                              ) : section === "Synonyms" || section === "Vocabulary" ? (
                                <>Detected {questions.length} vocabulary gap{questions.length > 1 ? "s" : ""}. Expand your <strong>word bank and context understanding</strong>.</>
                              ) : section === "Comparison" ? (
                                <>Found {questions.length} error{questions.length > 1 ? "s" : ""} in comparative forms. Review <strong>positive, comparative, and superlative degrees</strong>.</>
                              ) : section === "Idioms" ? (
                                <>Missed {questions.length} idiom{questions.length > 1 ? "s" : ""}. Idioms require <strong>contextual learning and regular exposure</strong>.</>
                              ) : (
                                <>Found {questions.length} area{questions.length > 1 ? "s" : ""} needing attention. Review the fundamental concepts and practice with varied examples.</>
                              )}
                            </p>
        ...
        <div className="max-w-5xl mx-auto">
          <NeumorphicCard className="w-full">
            <div className="p-6 md:p-4 text-center">
              ...
              <p className="text-lg text-muted-foreground mb-6">{testData.title}</p>
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-6">
                <div className="p-4 rounded-xl shadow-neu-inset bg-gradient-to-br from-card to-secondary/20">
                  <div className="text-2xl font-bold text-primary mb-1">{score.percentage}%</div>
                  <div className="text-xs text-muted-foreground">Score</div>
                </div>
                <div className="p-4 rounded-xl shadow-neu-inset bg-gradient-to-br from-card to-secondary/20">
                  <div className="text-2xl font-bold text-foreground mb-1">{score.correct}/{score.total}</div>
                  <div className="text-xs text-muted-foreground">Correct Answers</div>
                </div>
                <div className="p-4 rounded-xl shadow-neu-inset bg-gradient-to-br from-card to-secondary/20">
                  <div className="text-2xl font-bold text-foreground mb-1">{answeredQuestions}/{totalQuestions}</div>
              ...
              {(() => {
                ...
                return (
                  <motion.div
                    ...
                    className="mb-6"
                  >
                    <span className={`inline-block px-6 py-2 rounded-full text-white font-bold text-lg bg-gradient-to-r ${level.color} shadow-neu-lg`}>
                      {level.label} Level
                    </span>
                  </motion.div>
                );
              ...
              <div className="mb-6">
                {score.percentage >= 80 && (
                  <p className="text-base text-primary font-medium">🎉 Excellent work! You have a strong grasp of grammar!</p>
                )}
                {score.percentage >= 60 && score.percentage < 80 && (
                  <p className="text-base text-accent font-medium">👍 Good job! Keep practicing to improve further!</p>
                )}
                {score.percentage < 60 && (
                  <p className="text-base text-muted-foreground font-medium">💪 Keep learning! Practice makes perfect!</p>
                )}
              </div>
              ...
              {/* Only back to selection — no retry */}
              {onBackToSelection && (
                <NeumorphicButton variant="primary" onClick={onBackToSelection}>
